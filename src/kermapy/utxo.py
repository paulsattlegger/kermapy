...
def create_utxo_set(block: dict, objs: objects.Objects) -> dict:
    ...
    for tx_id in tx_ids:
        try:
            ...
            adjust_utxo_set_for_transaction(utxo_set, tx_id, stored_tx, objs)
        except KeyError:
            raise UtxoError(
...
def adjust_utxo_set_for_transaction(utxo_set: dict, tx_id: str, tx: dict, objs: objects.Objects) -> None:
    if "inputs" in tx:
